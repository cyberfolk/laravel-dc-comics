<footer>
    <section id="footer_up" class="py-5 position-relative">
        <div class="container">
            <div class="row">
                <div class="col-6">
                    <div class="row">
                        <div class="col-3">
                            <h2>DC COMICS</h2>
                            <ul>
                                <li><a href="#">Characters</a></li>
                                <li><a href="#">Comics</a></li>
                                <li><a href="#">Movies</a></li>
                                <li><a href="#">TV</a></li>
                                <li><a href="#">Games</a></li>
                                <li><a href="#">Videos</a></li>
                                <li><a href="#">News</a></li>
                            </ul>
                            <h2>SHOP</h2>
                            <ul>
                                <li><a href="#">Shop DC</a></li>
                                <li><a href="#">Shop DC Collectibles</a></li>
                            </ul>
                        </div>
                        <!-- /.col-3 -->

                        <div class="col-3">
                            <h2>DC</h2>
                            <ul>
                                <li><a href="#">Term Of Use</a></li>
                                <li><a href="#">Privacy polici (News)</a></li>
                                <li><a href="#">Ad Choices</a></li>
                                <li><a href="#">Advertising</a></li>
                                <li><a href="#">Jobs</a></li>
                                <li><a href="#">Subscriptions</a></li>
                                <li><a href="#">Talent Workshops</a></li>
                                <li><a href="#">CPSC Certificates</a></li>
                                <li><a href="#">Ratings</a></li>
                                <li><a href="#">Shop Help</a></li>
                                <li><a href="#">Contact Us</a></li>
                            </ul>
                        </div>
                        <!-- /.col-3 -->

                        <div class="col-3">
                            <h2>SITES</h2>
                            <ul>
                                <li><a href="#">DC</a></li>
                                <li><a href="#">MAD Magazine</a></li>
                                <li><a href="#">DC Kids</a></li>
                                <li><a href="#">DC Universe</a></li>
                                <li><a href="#">DC Power Visa</a></li>
                            </ul>
                        </div>
                        <!-- /.col-3 -->
                    </div>
                    <!-- /.row -->
                </div>
                <!-- /.col-6 -->
                <div class="col-6 d-flex justify-content-end">
                    <img class="position-absolute" src="{{Vite::asset('resources/img/dc-logo-bg.png')}}" alt="" />
                </div>
                <!-- /.col-6 -->
            </div>
            <!-- /.row -->
        </div>
        <!-- /.container -->
    </section>
    <!-- /#footer_up -->

    <section id="footer_down" class="position-relative">
        <div class="container">
            <div class="row">
                <div class="col-6">
                    <button class="py-2 px-3 fw-bold bg-transparent">SIGN-UP NOW!</button>
                </div>
                <!-- /.col-6 -->
                <div class="col-6">
                    <div class="d-flex justify-content-end align-items-center h-100 gap-4">
                        <div class="follow fs-5 fw-bold">FOLLOW US</div>
                        <ul class="list-inline d-flex gap-3 m-0">
                            <li>
                                <a href="#"><img src="{{Vite::asset('resources/img/footer-facebook.png')}}" alt="" /></a>
                            </li>
                            <li>
                                <a href="#"><img src="{{Vite::asset('resources/img/footer-twitter.png')}}" alt="" /></a>
                            </li>
                            <li>
                                <a href="#"><img src="{{Vite::asset('resources/img/footer-youtube.png')}}" alt="" /></a>
                            </li>
                            <li>
                                <a href="#"><img src="{{Vite::asset('resources/img/footer-pinterest.png')}}" alt="" /></a>
                            </li>
                            <li>
                                <a href="#"><img src="{{Vite::asset('resources/img/footer-periscope.png')}}" alt="" /></a>
                            </li>
                        </ul>
                    </div>
                </div>
                <!-- /.col-6 -->
            </div>
            <!-- /.row -->
        </div>
        <!-- /.container -->
    </section>
    <!-- /#footer_down -->
</footer>